...
SD2_sample_size_figure <- function() {
  parameters <- "low_rep/data_generation/SD2.txt"
  ...
  for (param_list in param_list_final) {
    ...
    inputDirectory <- file.path('Input', 'low_rep', generator)
    this_params_folder <- file.path(inputDirectory, inputString)
    outputDirectory <- file.path('Output', 'low_rep', generator)
  ...
  melted_df$tool <- case_when(melted_df$tool == 'Maaslin3CompAdjustFixEf' ~ 'Maaslin3 Median Adjusted Fixed Effects',
                              melted_df$tool == 'Maaslin3CompAdjustRanEf' ~ 'Maaslin3 Median Adjusted Random Effects',
                              melted_df$tool == 'Maaslin3FixEf' ~ 'MaAsLin 3 No adjustment Fixed Effects',
                              melted_df$tool == 'Maaslin3RanEf' ~ 'MaAsLin 3 No adjustment Random Effects')
  melted_df$tool <- factor(melted_df$tool, levels = c('MaAsLin 3 No adjustment Fixed Effects', 
                                                      "MaAsLin 3 No adjustment Random Effects", 
                                                      "Maaslin3 Median Adjusted Fixed Effects", 
                                                      "Maaslin3 Median Adjusted Random Effects"))
  ...
  plot_out <- ggplot(melted_df, aes(x = get(param_name), y = value, fill = tool)) + 
    ...
    scale_fill_manual(values=c("#104E8B", "#458B00", "#EEAD0E", "#68228B"),
                      breaks=c('MaAsLin 3 No adjustment Fixed Effects', 
                               "MaAsLin 3 No adjustment Random Effects", 
                               "Maaslin3 Median Adjusted Fixed Effects", 
                               "Maaslin3 Median Adjusted Random Effects"))
...
low_rep_SD2_sample_size_figure_maaslin3 <- function() {
  parameters <- "low_rep/data_generation/SD2.txt"
  ...
  melted_df$tool <- case_when(melted_df$tool == 'Maaslin3CompAdjustFixEf' ~ 'Maaslin3 Median Adjusted Fixed Effects',
                              melted_df$tool == 'Maaslin3CompAdjustRanEf' ~ 'Maaslin3 Median Adjusted Random Effects',
                              melted_df$tool == 'Maaslin3CompAdjustStrata' ~ 'Maaslin3 Median Adjusted Strata',
                              melted_df$tool == 'Maaslin3FixEf' ~ 'MaAsLin 3 No adjustment Fixed Effects',
                              melted_df$tool == 'Maaslin3RanEf' ~ 'MaAsLin 3 No adjustment Random Effects',
                              melted_df$tool == 'Maaslin3Strata' ~ 'MaAsLin 3 No adjustment Strata',)
  melted_df$tool <- paste0(melted_df$tool, " ", ifelse(melted_df$association_type == 'abundance', "Abundance", "Prevalence"))
  
  tool_vec <- c('MaAsLin 3 No adjustment Fixed Effects', 
                "MaAsLin 3 No adjustment Random Effects", 
                "MaAsLin 3 No adjustment Strata",
                "Maaslin3 Median Adjusted Fixed Effects", 
                "Maaslin3 Median Adjusted Random Effects",
                "Maaslin3 Median Adjusted Strata")
  
  melted_df$tool <- factor(melted_df$tool, levels = c(paste0(tool_vec, " ", "Abundance"),
                                                      paste0(tool_vec, " ", "Prevalence")))
  melted_df <- melted_df[!is.na(melted_df$tool),]
  melted_df <- melted_df[grepl('Median Adjusted', melted_df$tool),] # Median adjustment is just always better
  ...
  plot_out <- ggplot(melted_df, aes(x = get(param_name), y = value, fill = tool)) + 
    ...
    scale_fill_manual(values=c("#EE7600", "#68228B", "#8B1A1A", "#118811", "#0033AA", "#FFC0CB",
                               "#F7BB80", "#B491C5", "#C58D8D", "#11CC11", "#0099FF", "#AA8098"),
                      breaks=c(paste0(tool_vec, " ", "Abundance"),
                               paste0(tool_vec, " ", "Prevalence")))
  ggsave(paste0(figures_folder, 'low_rep_SD2_sample_size_figure_maaslin3.png'),
