...
workflow = Workflow(version="0.1", description="Evaluate low rep")
...
output = os.path.abspath(args.output.rstrip("low_rep/")) + "/"
...
def compute_generation_outputs(generator):
    if args.tmp:
        parameters_file = 'low_rep/data_generation/' + generator + '_tmp.txt'
        nIterations = 5
    else:
        parameters_file = 'low_rep/data_generation/' + generator + '.txt'
        nIterations = 100
    ...
    for param in param_list_final:
        ...
        new_depends_folder = output + 'Input/low_rep/' + generator + '/' + '_'.join([inputSubString, 
...
for generator in param_dict['generators']:
    if args.tmp:
        generate_command = 'python3 low_rep/data_generation/' + generator + '_workflow.py --parameters low_rep/data_generation/' + generator + '_tmp.txt --working-directory ' + output + ' --cores ' + str(cores) + ' --tmp'
    else:
        generate_command = 'python3 low_rep/data_generation/' + generator + '_workflow.py --parameters low_rep/data_generation/' + generator + '.txt --working-directory ' + output + ' --cores ' + str(cores)
...
def compute_running_outputs(generator, tool, param):
    ...
    new_depends_folder = output + 'Output/low_rep/' + generator + '/' + '_'.join([inputSubString, 
    param['metadataType'], param['nSubjects'], param['nPerSubject'], param['nMicrobes'], 
...
for generator in param_dict['generators']:
    if args.tmp:
        with open(str(working_directory + 'low_rep/data_generation/' + generator + '_tmp.txt'), 'r') as file:
            lines = file.readlines()
    else:
        with open(str(working_directory + 'low_rep/data_generation/' + generator + '.txt'), 'r') as file:
            lines = file.readlines()
