...
workflow = Workflow(version="0.1", description="Evaluate deep_sequencing")
...
output = os.path.abspath(args.output.removesuffix("deep_sequencing/")) + "/"
...
def compute_generation_outputs(generator):
    if args.tmp:
        parameters_file = 'deep_sequencing/data_generation/' + generator + '_tmp.txt'
        nIterations = 5
    else:
        parameters_file = 'deep_sequencing/data_generation/' + generator + '.txt'
        nIterations = 100
    ...
    for param in param_list_final:
        ...
        new_depends_folder = output + 'Input/deep_sequencing/' + generator + '/' + '_'.join([inputSubString, 
...
for generator in param_dict['generators']:
    if args.tmp:
        generate_command = 'python3 deep_sequencing/data_generation/' + generator + '_workflow.py --parameters deep_sequencing/data_generation/' + generator + '_tmp.txt --working-directory ' + output + ' --cores ' + str(cores) + ' --tmp'
    else:
        generate_command = 'python3 deep_sequencing/data_generation/' + generator + '_workflow.py --parameters deep_sequencing/data_generation/' + generator + '.txt --working-directory ' + output + ' --cores ' + str(cores)
...
def compute_running_outputs(generator, tool, param):
    ...
    new_depends_folder = output + 'Output/deep_sequencing/' + generator + '/' + '_'.join([inputSubString, 
    param['metadataType'], param['nSubjects'], param['nPerSubject'], param['nMicrobes'], 
    param['spikeMicrobes'], param['nMetadata'], param['effectSize'], param['effectPos'], 
...
for generator in param_dict['generators']:
    if args.tmp:
        with open(str(working_directory + 'deep_sequencing/data_generation/' + generator + '_tmp.txt'), 'r') as file:
            lines = file.readlines()
    else:
        with open(str(working_directory + 'deep_sequencing/data_generation/' + generator + '.txt'), 'r') as file:
            lines = file.readlines()
