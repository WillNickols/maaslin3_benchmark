...
groups_sample_size_figure <- function() {
  parameters <- "groups/data_generation/SD2.txt"
  ...
  for (param_list in param_list_final[c(1, 3, 5, 7, 9)]) {
    ...
    inputDirectory <- file.path('Input', 'group_evaluations', generator)
    this_params_folder <- file.path(inputDirectory, inputString)
    outputDirectory <- file.path('Output', 'group_evaluations', generator)
