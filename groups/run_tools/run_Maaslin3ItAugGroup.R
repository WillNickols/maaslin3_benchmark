...
nGroups<- opt$options$nGroups # Low-level parameter
...
options("scipen"=10)
inputString<-paste(inputSubString, metadataType, nSubjects, nPerSubject, nMicrobes, spikeMicrobes, nMetadata, effectSize, effectPos, readDepth, nGroups, nLevels, sep='_')
options("scipen"=5)
...
outputString <- paste(inputString, "Maaslin3ItAugGroup", sep="_")
  ...
  f <- foreach(i = reps, .packages = c("MASS", "stringi", 'dplyr', 'plyr', 'lme4', 'TcGSA')) %dopar% {
    ...
    if(length(ID)==length(unique(ID))){
      param_list <- list(input_data = abundance, input_metadata = metadata, min_abundance = 0, min_prevalence = 0.0, output = tmp_fit_out, 
                                    ...
                                    group_effects = omp_cols, save_scatter = FALSE, 
