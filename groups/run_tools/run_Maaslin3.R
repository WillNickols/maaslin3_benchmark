...
inputString<-paste(inputSubString, metadataType, nSubjects, nPerSubject, nMicrobes, spikeMicrobes, nMetadata, 
                   effectSize, effectPos, readDepth, nGroups, nLevels, sep='_')
options("scipen"=5)
...
if (!outputs_already_exist){
  ...
  f <- foreach(i = reps, .packages = c("MASS", "stringi", 'dplyr', 'plyr', 'lme4', 'TcGSA')) %dopar% {
    ...
    group_cols <- colnames(metadata)[apply(metadata, 2, function(x) {length(x) - length(unique(x)) > 5})]
    for (group_col in group_cols) {
      metadata[,group_col] <- as.factor(metadata[,group_col])
    }
    group_cols <- group_cols[group_cols != 'ID']
