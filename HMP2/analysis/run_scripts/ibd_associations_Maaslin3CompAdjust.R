...
if (opt$options$dataset == 'taxa') {
    if (opt$options$version == 3) {
        tmp_fit_out <- paste0(gsub("/$", "", analysisDirectory), "/fit_out_Maaslin3CompAdjust_3")
    } else {
        tmp_fit_out <- paste0(gsub("/$", "", analysisDirectory), "/fit_out_Maaslin3CompAdjust_4")
    }
} else {
    tmp_fit_out <- paste0(gsub("/$", "", analysisDirectory), "/fit_out_Maaslin3CompAdjust_mbx")
}
...
write.table(fit_out_joint, paste0(gsub("/$", "", analysisDirectory), "/results/", ifelse(opt$options$dataset == 'taxa', '', 'mbx_'), "v", opt$options$version, "_ibd_associations_Maaslin3CompAdjust.tsv"), row.names = F, sep='\t')
