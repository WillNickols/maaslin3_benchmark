...
# Subset based on age
if (opt$options$dataset == 'under16') {
    metadata <- metadata %>%
        dplyr::filter(consent_age < 16)
    taxa_table <- taxa_table[,colnames(taxa_table) %in% metadata$sample]
    taxa_table <- taxa_table[rowMeans(taxa_table) != 0,]
} else if (opt$options$dataset == 'atleast16') {
    metadata <- metadata %>%
        dplyr::filter(consent_age >= 16)
    taxa_table <- taxa_table[,colnames(taxa_table) %in% metadata$sample]
    taxa_table <- taxa_table[rowMeans(taxa_table) != 0,]
} else {
    stop("dataset must be under16 or atleast16")
}

tmp_fit_out <- paste0(gsub("/$", "", analysisDirectory), "/fit_out_Maaslin2_", opt$options$dataset, "_", opt$options$version)
...
fit_out <- Maaslin2::Maaslin2(taxa_table, metadata, min_abundance = 0, min_prevalence = 0, output = tmp_fit_out, 
                              ...
                              random_effects = c("participant_id"), fixed_effects = c('diagnosis', 'dysbiosis_state', 'Antibiotics', 'reads_filtered'), 
...
write.table(fit_out, paste0(gsub("/$", "", analysisDirectory), "/results/", 
                                  "v", opt$options$version, "_",
                                  opt$options$dataset, "_Maaslin2.tsv"), 
            row.names = F, sep='\t')
