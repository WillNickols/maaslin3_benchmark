...
tmp_fit_out <- paste0(gsub("/$", "", analysisDirectory), "/fit_out_Maaslin3_", opt$options$dataset, "_", opt$options$version)
...
fit_out <- maaslin3::maaslin3(input_data = taxa_table,
                              ...
                              normalization = 'TSS', 
                              transform = 'LOG', 
                              formula = '~ diagnosis + dysbiosis_state + Antibiotics + reads_filtered + (1 | participant_id)', 
                              plot_summary_plot = T, 
                              plot_associations = T, 
...
write.table(fit_out_joint, paste0(gsub("/$", "", analysisDirectory), "/results/", 
                                  "v", opt$options$version, "_",
                                  opt$options$dataset, "_Maaslin3.tsv"), 
            row.names = F, sep='\t')
