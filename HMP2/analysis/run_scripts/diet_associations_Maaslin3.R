...
tmp_fit_out <- paste0(gsub("/$", "", analysisDirectory), "/fit_out_Maaslin3_", input_food, "_", opt$options$ordered, "_", opt$options$version)
...
fit_out <- maaslin3::maaslin3(input_data = taxa_table,
                   ...
                   normalization = 'TSS', 
                   transform = 'LOG', 
                   formula = ifelse(opt$options$ordered == 'ordered', 
                                    '~ dysbiosis_state + Antibiotics + consent_age + reads_filtered + (1|participant_id) + ordered(food_group)', 
                                    '~ dysbiosis_state + Antibiotics + consent_age + reads_filtered + (1|participant_id) + group(food_group)'), 
                   plot_summary_plot = F, 
                   plot_associations = F, 
...
write.table(fit_out_joint, paste0(gsub("/$", "", analysisDirectory), "/results/", ifelse(opt$options$dataset == 'taxa', '', 'mbx_'), 
                                  "v", opt$options$version, "_",
                                  ifelse(opt$options$ordered == 'ordered', 'ordered', 'group'), 
