...
def compute_outputs(food, ordered, dataset, version):
    if dataset == 'taxa':
        return output + 'results/' + 'v' + str(version) + '_' + ordered + "_food_associations_" + food + ".tsv"

dataset = 'taxa'
for food in foods:
    for ordered in ['ordered', 'group']:
        for version in [3,4]:
            new_command = 'Rscript run_scripts/' + 'diet_associations_Maaslin3.R' + ' --nCores ' + str(cores) + ' --workingDirectory ' + args.workingDirectory + ' --analysisDirectory ' + output + ' --food ' + food + ' --ordered ' + ordered  + ' --dataset ' + dataset + ' --version ' + str(version)

            if not os.path.exists(compute_outputs(food, ordered, dataset, version)):
                workflow.add_task_gridable(actions=new_command,
                depends=[output + 'data/hmp2_metadata_2018-08-20.csv', output + 'data/metaphlan4_taxonomic_profiles.tsv', output + 'data/metaphlan3_taxonomic_profiles.tsv', output + 'data/annotations_hmp2.csv', output + 'data/intensities_hmp2.csv'],
                targets=compute_outputs(food, ordered, dataset, version),
                time=time,
                mem=memory,
                cores=cores,
                partition=partition
