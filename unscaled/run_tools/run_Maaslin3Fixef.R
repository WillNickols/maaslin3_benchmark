...
outputString <- paste(inputString, "Maaslin3Fixef", sep="_")
  ...
  f <- foreach(i = reps, .packages = c("MASS", "stringi", 'dplyr', 'plyr', 'lme4')) %dopar% {
    ...
    if(length(ID)==length(unique(ID))){
      ...
      fit_out <- maaslin3::maaslin3(input_data = abundance, 
                                    ...
                                    normalization = 'TSS', 
                                    transform = 'LOG',
                                    fixed_effects = colnames(metadata)[!colnames(metadata) %in% c("read_depth")],
                                    median_comparison_abundance = T, 
                                    median_comparison_prevalence = F,
                                    plot_summary_plot = F, 
                                    plot_associations = F, 
