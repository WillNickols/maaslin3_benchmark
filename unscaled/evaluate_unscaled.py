...
def compute_generation_outputs(generator):
    if args.tmp:
        parameters_file = 'unscaled/data_generation/' + generator + '_tmp.txt'
        nIterations = 5
    else:
        parameters_file = 'unscaled/data_generation/' + generator + '.txt'
        nIterations = 100
...
for generator in param_dict['generators']:
    if args.tmp:
        generate_command = 'python3 unscaled/data_generation/' + generator + '_workflow.py --parameters unscaled/data_generation/' + generator + '_tmp.txt --working-directory ' + output + ' --cores ' + str(cores) + ' --tmp'
    else:
        generate_command = 'python3 unscaled/data_generation/' + generator + '_workflow.py --parameters unscaled/data_generation/' + generator + '.txt --working-directory ' + output + ' --cores ' + str(cores)
...
for generator in param_dict['generators']:
    if args.tmp:
        with open(str(working_directory + 'unscaled/data_generation/' + generator + '_tmp.txt'), 'r') as file:
            lines = file.readlines()
    else:
        with open(str(working_directory + 'unscaled/data_generation/' + generator + '.txt'), 'r') as file:
            lines = file.readlines()
