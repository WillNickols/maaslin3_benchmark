...
SD2_threshold_roc_figure_maaslin3 <- function() {
    parameters <- "unscaled/data_generation/SD2.txt"
    ...
    ggsave(paste0(figures_folder, 'SD2_threshold_roc_figure_maaslin3.png'),
           plot = plot_out, width = 5, height = 5)
}
SD2_threshold_roc_figure_maaslin3()

SD2_improvement_roc_figure_maaslin3 <- function() {
    parameters <- "unscaled/data_generation/SD2.txt"
    ...
    melted_df <- melted_df[!is.na(melted_df$tool),]
    melted_df$alpha <- ifelse(melted_df$nSubjects == 100, 0.5, 1)
    melted_df <- melted_df[melted_df$association_type == 'abundance',]
    
    plot_out <- ggplot(melted_df, 
                       aes(x = Recall, y = Precision, color = tool, alpha = nSubjects,
                           group = interaction(tool, nSubjects))) +
        scale_alpha_manual(values = c("100" = 0.5, "1000" = 1),
                           guide = "legend") + 
    ...
    ggsave(paste0(figures_folder, 'SD2_improvement_abundance_roc_figure_maaslin3.png'),
           plot = plot_out, width = 9, height = 5)
    
    melted_df <- maaslin3_results_df %>%
        group_by(tool, association_type, thresholds, nSubjects, allow_abun_to_prev) %>%
        summarise(Recall = mean(Recall, na.rm=T), Precision = mean(Precision, na.rm=T))
    ...
    ggsave(paste0(figures_folder, 'SD2_improvement_precision_roc_figure_maaslin3.png'),
           plot = plot_out, width = 9, height = 5)
}
SD2_improvement_roc_figure_maaslin3()

SD2_sample_size_prev_recall <- function() {
    parameters <- "unscaled/data_generation/SD2.txt"
