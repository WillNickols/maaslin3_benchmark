...
if (opt$options$dataset == 'taxa') {
  tmp_fit_out <- paste0(gsub("/$", "", analysisDirectory), "/fit_out_Maaslin3CompAdjust")
} else {
...
write.table(fit_out_joint, paste0(gsub("/$", "", analysisDirectory), "/results/", ifelse(opt$options$dataset == 'taxa', '', 'mbx_'), "ibd_associations_Maaslin3CompAdjust.tsv"), row.names = F, sep='\t')
