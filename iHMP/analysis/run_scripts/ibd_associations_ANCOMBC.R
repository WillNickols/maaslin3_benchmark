...
both_tse <- TreeSummarizedExperiment(
  assays = assays,
  colData = smd)
ancombc_out <- ancombc2(both_tse, fix_formula = 'diagnosis + dysbiosis_state + Antibiotics + consent_age',
                        rand_formula = '(1|participant_id)', p_adj_method = "BH", alpha = 0.1, struc_zero = T,
                        group = c('dysbiosis_state'), n_cl = nCores)
...
write.table(outputs, paste0(gsub("/$", "", analysisDirectory), "/results/", ifelse(opt$options$dataset == 'taxa', '', 'mbx_'), "ibd_associations_ANCOMBC.tsv"), row.names = F, sep='\t')
