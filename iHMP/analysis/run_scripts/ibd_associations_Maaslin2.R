...
if (opt$options$dataset == 'taxa') {
  ...
  tmp_fit_out <- paste0(gsub("/$", "", analysisDirectory), "/tmp_fit_out_Maaslin2_mbx")
}
...
write.table(fit_out, paste0(gsub("/$", "", analysisDirectory), "/results/", ifelse(opt$options$dataset == 'taxa', '', 'mbx_'), "ibd_associations_Maaslin2.tsv"), row.names = F, sep='\t')
