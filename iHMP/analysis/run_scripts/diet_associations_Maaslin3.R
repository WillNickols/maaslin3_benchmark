...
if (opt$options$dataset == 'taxa') {
  keep_cols <- c('External ID', 'Participant ID', 'week_num', 'site_name', 'Age at diagnosis',
                 ...
                 colnames(metadata)[c(52:83, 85:111)],
                 'race', 'sex', 'BMI')
} else {
  keep_cols <- c('External ID', 'Participant ID', 'week_num', 'site_name', 'Age at diagnosis',
                 'Education Level', 'Occupation', 'consent_age', 'diagnosis',
                 colnames(metadata)[c(48:68, 77)], 
                 'race', 'sex', 'BMI', "Antibiotics")
}
...
tmp_fit_out <- paste0(gsub("/$", "", analysisDirectory), "/tmp_fit_out_Maaslin3ItAug_", opt$options$food, opt$options$omp)

if (opt$options$dataset == 'taxa') {
  tmp_fit_out <- paste0(gsub("/$", "", analysisDirectory), "/tmp_fit_out_Maaslin3ItAug_", opt$options$food, opt$options$omp)
} else {
  tmp_fit_out <- paste0(gsub("/$", "", analysisDirectory), "/tmp_fit_out_Maaslin3ItAug_mbx",  opt$options$food, opt$options$omp)
}

input_food <- opt$options$food
selected_food <- case_when(input_food == 'soft_drinks' ~ "Soft drinks, tea or coffee with sugar (corn syrup, maple syrup, cane sugar, etc)",
                           input_food == 'diet_soft_drinks' ~ "Diet soft drinks, tea or coffee with sugar (Stevia, Equal, Splenda etc)",
                           input_food == 'fruit_juice' ~ "Fruit juice (orange, apple, cranberry, prune etc.)",
                           input_food == 'water' ~ "Water",
                           input_food == 'alcohol' ~ "Alcohol (beer, brandy, spirits, hard liquor, wine, aperitif, etc.)",
                           input_food == 'yogurt' ~ "Yogurt or other foods containing active bacterial cultures (kefir, sauerkraut)",
                           input_food == 'dairy' ~ "Dairy (milk, cream, ice cream, cheese, cream cheese)",
                           input_food == 'probiotic' ~ "Probiotic",
                           input_food == 'fruits' ~ "Fruits (no juice) (Apples, raisins, bananas, oranges, strawberries, blueberries",
                           input_food == 'vegetables' ~ "Vegetables (salad, tomatoes, onions, greens, carrots, peppers, green beans, etc)",
                           input_food == 'beans' ~ "Beans (tofu, soy, soy burgers, lentils, Mexican beans, lima beans etc)",
                           input_food == 'whole_grains' ~ "Whole grains (wheat, oats, brown rice, rye, quinoa, wheat bread, wheat pasta)",
                           input_food == 'starch' ~ "Starch (white rice, bread, pizza, potatoes, yams, cereals, pancakes, etc.)",
                           input_food == 'eggs' ~ "Eggs",
                           input_food == 'processed_meat' ~ "Processed meat (other red or white meat such as lunch meat, ham, salami, bologna",
                           input_food == 'red_meat' ~ "Red meat (beef, hamburger, pork, lamb)",
                           input_food == 'white_meat' ~ "White meat (chicken, turkey, etc.)",
                           input_food == 'shellfish' ~ "Shellfish (shrimp, lobster, scallops, etc.)",
                           input_food == 'fish' ~ "Fish (fish nuggets, breaded fish, fish cakes, salmon, tuna, etc.)",
                           input_food == 'sweets' ~ "Sweets (pies, jam, chocolate, cake, cookies, etc.)",
                           input_food == 'tea' ~ "Tea or coffee no sugar and no sugar replacement")

metadata$food_group <- metadata[,selected_food]
metadata <- metadata[metadata$diagnosis == 'CD',]
metadata <- metadata[metadata$food_group != '',]
metadata$food_group <- factor(metadata$food_group, levels = c("No, I did not consume these products in the last 7 days",
...
if (opt$options$dataset == 'taxa') {
  param_list <- list(input_data = taxa_table, input_metadata = metadata, min_abundance = 0, min_prevalence = 0, output = tmp_fit_out, 
                     ...
                     formula = ifelse(opt$options$omp == 'omp', 
                                      '~ dysbiosis_state + Antibiotics + consent_age + (1|participant_id) + omp(food_group)', 
                                      '~ dysbiosis_state + Antibiotics + consent_age + (1|participant_id) + gomp(food_group)'), 
                     save_scatter = FALSE, 
...
write.table(fit_out_joint, paste0(gsub("/$", "", analysisDirectory), "/results/", ifelse(opt$options$dataset == 'taxa', '', 'mbx_'), 
                                  ifelse(opt$options$omp == 'omp', 'omp', 'gomp'), 
                                  "_food_associations_", input_food, ".tsv"), row.names = F, sep='\t')
