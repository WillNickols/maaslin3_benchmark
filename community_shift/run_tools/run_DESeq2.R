...
remove(list = ls())
package_vec = c("devtools", "pkgmaker", "optparse", "DESeq2",
...
RandomEffect<-opt$options$RandomEffect
metadataType <- opt$options$metadataType
nSubjects <- opt$options$nSubjects
nPerSubject<-opt$options$nPerSubject
nMicrobes <- opt$options$nMicrobes
spikeMicrobes <- opt$options$spikeMicrobes
nMetadata<- opt$options$nMetadata
effectSize<- opt$options$effectSize
effectPos<- opt$options$effectPos
readDepth<- opt$options$readDepth
depthConfound<- opt$options$depthConfound
propAbun<- opt$options$propAbun
zeroInflate<- opt$options$zeroInflate
noParallel<-opt$options$noParallel
nIterations<- opt$options$nIterations
rSeed<- opt$options$rSeed
...
options("scipen"=10)
inputString<-paste(inputSubString, metadataType, nSubjects, nPerSubject, nMicrobes, spikeMicrobes, nMetadata, effectSize, effectPos, readDepth, depthConfound, propAbun, zeroInflate, sep='_')
options("scipen"=5)

inputDirectory <- file.path(workingDirectory, 'Input', 'community_shift', generator)
...
outputDirectory <- file.path(workingDirectory, 'Output', 'community_shift', generator)
outputString <- paste(inputString, "DESeq2", sep="_")
...
if (!outputs_already_exist){
    ...
    f <- foreach(i = reps, .packages = c("MASS", "stringi", "dplyr", "plyr", "DESeq2")) %dopar% {
        ...
        if ("ID" %in% colnames(metadata) & length(unique(metadata$ID)) != length(metadata$ID)) {
            dds <- DESeqDataSetFromMatrix(countData = abundance, colData = metadata, design = formula(paste0("~ ", paste0(c(fixed_effects, "ID"), collapse = " + "))))
        } else {
            dds <- DESeqDataSetFromMatrix(countData = abundance, colData = metadata, design = formula(paste0("~ ", paste0(fixed_effects, collapse = " + "))))
        }
        dds <- estimateSizeFactors(dds, type="poscounts")
        dds <- DESeq(dds)
        res_names <- resultsNames(dds)
        if ("Intercept" %in% res_names) {
            res_names <- res_names[res_names != "Intercept"]
        }
        out <- NULL
        for(coef in res_names) {
            res <- results(dds, name = coef)
            tmp <- data.frame(taxon = rownames(res), 
                              metadata = coef, 
                              effect_size = res$log2FoldChange, 
                              pval = res$pvalue, 
                              associations = "abundance")
            if(is.null(out)) { out <- tmp } else { out <- rbind(out, tmp) }
        }
        sink()
        
        outputs <- out
        outputs$qval = p.adjust(outputs$pval, method = "BH")
        return(outputs)
    }
