...
workflow.add_argument('parameters', type=str, help="Specify the parameters")
...
for generator in param_dict['generators']:
    generate_command = '''{a} && {b}'''.format(
        a = 'python3 data_generation/' + generator + '_workflow.py --parameters ' + generator + '.txt --working-directory ' + output + ' --cores ' + str(cores),
        b = 'touch [targets[0]]'
        )
...
run_tools_command = '''{a} && {b}'''.format(
    a = 'python3 run_tools/workflow.py --generators ' + ','.join(param_dict['generators']) + ' --working-directory ' + output + ' --cores ' + str(cores),
    b = 'touch [targets[0]]'
    )
