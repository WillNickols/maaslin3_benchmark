...
remove(list = ls())
package_vec = c("devtools", "phyloseq", "pkgmaker", "optparse", 
...
make_option(
  ...
  type = "integer"),
make_option(
  c("-c", "--nCores"), default=1, # c stands for how many cores to be used in the analysis
  type = "integer"),
make_option(
...
pkgmaker::source_files(paste(workingDirectory, '/library/ANCOM_BC_generator_helper_functions.R', sep=''))
...
if (!outputs_already_exist){
  ...
  simlist <- trigger_ANCOM_BC_Simulator(metadataType=metadataType,
                                        nSubjects=nSubjects,
                                        nMicrobes=nMicrobes,
