...
remove(list = ls())
package_vec = c("devtools", "SimSeq", "pkgmaker", "optparse", 
...
pkgmaker::source_files(paste(workingDirectory, '/library/SimSeq_helper_functions.R', sep=''))
...
inputDirectory <- file.path(workingDirectory, 'Input', 'general_evaluations', 'SimSeq')
...
if (!outputs_already_exist){
  ...
  simlist<-trigger_SimSeq_Simulator(metadataType=metadataType,
                                    nSubjects=nSubjects,
                                    nMicrobes=nMicrobes,
