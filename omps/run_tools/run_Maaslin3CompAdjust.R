...
options("scipen"=10)
inputString<-paste(inputSubString, metadataType, nSubjects, nPerSubject, nMicrobes, spikeMicrobes, nMetadata, 
                   effectSize, effectPos, readDepth, nOmps, nLevels, sep='_')
options("scipen"=5)
  ...
  f <- foreach(i = reps, .packages = c("MASS", "stringi", 'dplyr', 'plyr', 'lme4', 'TcGSA')) %dopar% {
    ...
    if(length(ID)==length(unique(ID))){
      fit_out <- maaslin3::maaslin3(input_data = abundance, 
                         ...
                         normalization = 'TSS', 
                         transform = 'LOG',
                         fixed_effects = colnames(metadata)[!colnames(metadata) %in% c(omp_cols, "ID", "read_depth")], 
                         ordered_effects = omp_cols,
    ...
    fit_out_lm <- fit_out_lm[c("feature", "name", "coef", "pval_individual", "error", "qval_individual", "pval_joint", "qval_joint")]
    fit_out_lm$association <- "abundance"
    
    fit_out_binary <- fit_out$fit_data_prevalence$results
    fit_out_binary <- fit_out_binary[c("feature", "name", "coef", "pval_individual", "error", "qval_individual", "pval_joint", "qval_joint")]
