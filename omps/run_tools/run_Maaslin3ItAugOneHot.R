...
outputString <- paste(inputString, "Maaslin3ItAugOneHot", sep="_")
...
if (!outputs_already_exist){
  ...
  f <- foreach(i = reps, .packages = c("MASS", "stringi", 'dplyr', 'plyr', 'lme4')) %dopar% {
    ...
    metadata <- metadata[,!colnames(metadata) %in% omp_cols, drop = F]
    for (omp_col in omp_cols) {
      metadata <- cbind(metadata,
                        fill_left_of_ones(model.matrix(formula(
                          paste0('~ ', paste0(omp_col, collapse = '+'))), metadata_org)[,-1]))
    }
