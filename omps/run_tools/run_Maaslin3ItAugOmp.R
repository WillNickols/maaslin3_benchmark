...
nOmps<- opt$options$nOmps # Low-level parameter
nLevels<- opt$options$nLevels # Low-level parameter
...
options("scipen"=10)
inputString<-paste(inputSubString, metadataType, nSubjects, nPerSubject, nMicrobes, spikeMicrobes, nMetadata, effectSize, effectPos, readDepth, nOmps, nLevels, sep='_')
options("scipen"=5)
...
outputString <- paste(inputString, "Maaslin3ItAugOmp", sep="_")
...
if (!outputs_already_exist){
  ...
  f <- foreach(i = reps, .packages = c("MASS", "stringi", 'dplyr', 'plyr', 'lme4', 'TcGSA')) %dopar% {
    ...
    omp_cols <- colnames(metadata)[apply(metadata, 2, function(x) {length(x) - length(unique(x)) > 5})]
    for (omp_col in omp_cols) {
      metadata[,omp_col] <- as.factor(metadata[,omp_col])
    }
    omp_cols <- omp_cols[omp_cols != 'ID']
    ...
    if(length(ID)==length(unique(ID))){
      param_list <- list(input_data = abundance, input_metadata = metadata, min_abundance = 0, min_prevalence = 0.0, output = tmp_fit_out, 
                                    ...
                                    fixed_effects = colnames(metadata)[!colnames(metadata) %in% c(omp_cols, "ID")], 
                                    omp_effects = omp_cols, save_scatter = FALSE, 
                                    save_models = F, plot_heatmap = F, plot_scatter = F, max_significance = 0.1, augment = TRUE, iterative_mode = TRUE)
    } else{
      param_list <- list(input_data = abundance, input_metadata = metadata, min_abundance = 0, min_prevalence = 0.0, output = tmp_fit_out, 
                                    min_variance = 0, normalization = 'TSS', transform = 'log', analysis_method = 'LM', 
                                    random_effects = "ID", fixed_effects = colnames(metadata)[!colnames(metadata) %in% c(omp_cols, "ID")], 
                                    omp_effects = omp_cols, save_scatter = FALSE, 
    ...
    fit_out_lm <- fit_out_lm[c("feature", "name", "coef", "pval_single", "error", "qval_single", "pval_joint", "qval_joint")]
    fit_out_lm$association <- "abundance"
    
    fit_out_binary <- fit_out$fit_data_binary$results
    fit_out_binary <- fit_out_binary[c("feature", "name", "coef", "pval_single", "error", "qval_single", "pval_joint", "qval_joint")]
